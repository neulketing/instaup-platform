...
const APITestPanel = () => {
  ...
  const testEndpoint = async (endpoint: string, method: 'GET' | 'POST' = 'GET', body?: any): Promise<TestResult> => {
    ...
    try {
      const baseUrl = import.meta.env.VITE_BACKEND_API_URL || 'https://instaup-production.up.railway.app'
  ...
  const runAllTests = async () => {
    ...
    const tests = [
      ...
      { endpoint: '/api/auth/register', method: 'POST' as const, body: { email: 'test@example.com', password: 'test123', nickname: 'tester' } },
    ]
  ...
  // 개별 테스트 실행
  const runSingleTest = async (endpoint: string, method: 'GET' | 'POST' = 'GET') => {
    const result = await testEndpoint(endpoint, method)
    setTestResults(prev => {
      const filtered = prev.filter(r => !(r.endpoint === endpoint && r.method === method))
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
      <div className="flex justify-between items-center mb-6">
        <h3 className="text-lg font-semibold text-gray-900">🧪 API 연동 테스트</h3>
      ...
      <div className="space-y-3">
        {testResults.map((result, index) => (
          <div key={`${result.endpoint}-${result.method}-${index}`} className="border border-gray-200 rounded-lg p-4">
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-3">
                <span className={`px-2 py-1 text-xs font-medium rounded-full ${getStatusColor(result.status)}`}>
                  {result.status.toUpperCase()}
                </span>
                <span className="font-mono text-sm text-gray-600">{result.method}</span>
      ...
      <div className="mt-6 pt-4 border-t border-gray-200">
        <div className="text-xs text-gray-600">
          <div><strong>Backend URL:</strong> {import.meta.env.VITE_BACKEND_API_URL || 'https://instaup-production.up.railway.app'}</div>
