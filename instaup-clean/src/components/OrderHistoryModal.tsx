...
import { type OrderItem, OrderStatus, type OrderFilter } from '../types/orders'
import { exportOrdersToExcel, exportFilteredOrders, exportOrderStatsToExcel } from '../utils/excelExport'
...
export default function OrderHistoryModal({
  ...
  useEffect(() => {
    if (isOpen && isLoggedIn && userSession) {
      ...
      const unsubscribe = orderService.subscribe((updatedOrders) => {
        if (autoRefresh) {
          const userOrders = updatedOrders.filter(order => order.userId === userSession.id)
  ...
  const loadOrders = async () => {
    ...
    try {
      ...
      const filter: OrderFilter = {
        ...(selectedFilter !== 'all' && { status: selectedFilter as OrderStatus }),
        ...(searchTerm && { search: searchTerm })
      }

      const allOrders = orderService.getOrders(filter)
      const userOrders = allOrders.filter(order => order.userId === userSession.id)
      setOrders(userOrders)
    } catch (error) {
  ...
  const handleExportExcel = () => {
    ...
    const filterName = selectedFilter === 'all' ? '전체' : getStatusText(selectedFilter as OrderStatus)
  ...
  const getStatusColor = (status: OrderStatus) => {
    switch (status) {
      ...
      case OrderStatus.IN_PROGRESS: return 'text-blue-600 bg-blue-50 border-blue-200'
      case OrderStatus.PROCESSING: return 'text-purple-600 bg-purple-50 border-purple-200'
      ...
      case OrderStatus.REFUNDED: return 'text-gray-600 bg-gray-50 border-gray-200'
      case OrderStatus.PAUSED: return 'text-orange-600 bg-orange-50 border-orange-200'
  ...
  const getStatusText = (status: OrderStatus) => {
    switch (status) {
      ...
      case OrderStatus.IN_PROGRESS: return '진행중'
      ...
      case OrderStatus.PAUSED: return '일시정지'
  ...
  const getStatusIcon = (status: OrderStatus) => {
    switch (status) {
      ...
      case OrderStatus.IN_PROGRESS: return '⚡'
  ...
  const orderStats = userSession ? orderService.getOrderStats(userSession.id) : null
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm opacity-90">실시간 주문 현황 및 진행률을 확인하세요</p>
              </div>
            </div>
            <div className="flex items-center gap-4">
              <button
                ...
                className={`px-3 py-1 rounded-lg text-xs font-medium transition-colors ${
                  autoRefresh ? 'bg-green-500 text-white' : 'bg-white bg-opacity-20 text-white'
                }`}
              >
        ...
        <div className="p-6">
          ...
          {orderStats && (
            <div className="grid grid-cols-4 gap-4 mb-6">
              ...
              <div className="bg-gradient-to-br from-purple-50 to-pink-50 border border-purple-200 rounded-lg p-4 text-center">
                <div className="text-2xl font-bold text-purple-600">{Math.round(orderStats.completionRate)}%</div>
          ...
          <div className="mb-6 space-y-4">
            ...
            <div className="flex gap-2 flex-wrap">
              {[
                ...
                { key: OrderStatus.PENDING, label: '대기중', count: orders.filter(o => o.status === OrderStatus.PENDING).length },
                { key: OrderStatus.PROCESSING, label: '처리중', count: orders.filter(o => o.status === OrderStatus.PROCESSING).length },
                { key: OrderStatus.IN_PROGRESS, label: '진행중', count: orders.filter(o => o.status === OrderStatus.IN_PROGRESS).length },
                { key: OrderStatus.COMPLETED, label: '완료', count: orders.filter(o => o.status === OrderStatus.COMPLETED).length },
                { key: OrderStatus.CANCELLED, label: '취소됨', count: orders.filter(o => o.status === OrderStatus.CANCELLED).length },
              ].map(filter => (
                <button
                  key={filter.key}
          ...
          {isLoading ? (
            <div className="flex items-center justify-center py-12">
              ...
              <span className="ml-3 text-gray-600">주문 내역을 불러오는 중...</span>
            </div>
          ) : filteredOrders.length === 0 ? (
            <div className="text-center py-12">
              <div className="text-6xl mb-4">📦</div>
              <h3 className="text-lg font-bold text-gray-900 mb-2">
                {selectedFilter === 'all' ? '주문 내역이 없습니다' : `${getStatusText(selectedFilter as OrderStatus)} 주문이 없습니다`}
              </h3>
            ...
            <div className="space-y-4">
              {filteredOrders.map((order) => (
                <div key={order.id} className="bg-white rounded-xl border-2 border-gray-200 p-6 hover:shadow-lg transition-all hover:border-[#22426f]">
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-3">
                        <span className="text-2xl">{getPlatformIcon(order.platform)}</span>
                        <h3 className="font-bold text-lg text-gray-900">{order.serviceName}</h3>
                      ...
                      <div className="grid grid-cols-2 gap-4 text-sm text-gray-600 mb-3">
                        ...
                        <div>
                          ...
                          <span className={`ml-1 font-bold ${
                            order.quality === 'vip' ? 'text-purple-600' :
                            order.quality === 'premium' ? 'text-blue-600' : 'text-green-600'
                          }`}>
                            {order.quality.toUpperCase()}
                          </span>
                        </div>
                        <div>
                          <span className="font-medium">주문일시:</span> {new Date(order.createdAt).toLocaleString()}
                        </div>
                        <div>
                          <span className="font-medium">예상완료:</span> {order.estimatedCompletionTime}
                        </div>
                      </div>

                      <div className="bg-gray-50 rounded-lg p-3 mb-3">
                        <p className="text-sm text-gray-600 break-all">
                          <span className="font-medium">대상 URL:</span> {order.targetUrl}
                        </p>
                      </div>
                    </div>
                  ...
                  {/* 진행률 표시 */}
                  {[OrderStatus.PROCESSING, OrderStatus.IN_PROGRESS].includes(order.status) && (
                    <div className="mb-4">
                      <div className="flex justify-between text-sm mb-2">
                        <span className="text-gray-600 font-medium">진행률</span>
                        <div className="text-right">
                          <span className="font-bold text-[#22426f]">{order.progressPercentage}%</span>
                      ...
                      <div className="mt-2 flex items-center gap-2">
                        ...
                        <span className="text-sm text-blue-600 font-medium">
                          {orderService.getOrderProgress(order.id)[0]?.message || '진행중...'}
                        </span>
                      </div>
                    </div>
                  )}
                  ...
                  {order.status === OrderStatus.COMPLETED && (
                    <div className="mb-4 bg-green-50 border border-green-200 rounded-lg p-4">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <span className="font-bold text-green-800">주문 완료!</span>
                      </div>
                      <div className="grid grid-cols-2 gap-4 text-sm">
                        <div>
                          <span className="text-green-600 font-medium">완료일시:</span>
                          <span className="ml-2">{order.completedAt ? new Date(order.completedAt).toLocaleString() : '-'}</span>
                        </div>
                        <div>
                          <span className="text-green-600 font-medium">리필보장:</span>
                          <span className="ml-2 font-bold">{order.refillGuarantee}일</span>
                        </div>
                      </div>
                    </div>
                  )}
                  ...
                  <div className="flex items-center justify-between pt-4 border-t border-gray-200">
                    <div className="text-sm text-gray-500">
                      마지막 업데이트: {new Date(order.updatedAt).toLocaleString()}
                    </div>
                    <div className="flex gap-2">
                      {order.status === OrderStatus.COMPLETED && !order.isRefillRequest && (
                        <button
                ...
                <div className="space-y-6">
                  ...
                  <div className="bg-gray-50 rounded-lg p-4">
                    ...
                    <div className="grid grid-cols-2 gap-3 text-sm">
                      <div><span className="font-medium">주문번호:</span> {selectedOrder.id}</div>
                      <div><span className="font-medium">서비스:</span> {selectedOrder.serviceName}</div>
                      <div><span className="font-medium">플랫폼:</span> {selectedOrder.platform}</div>
                      <div><span className="font-medium">수량:</span> {selectedOrder.quantity.toLocaleString()}개</div>
                      <div><span className="font-medium">품질:</span> {selectedOrder.quality.toUpperCase()}</div>
                      <div><span className="font-medium">가격:</span> {selectedOrder.finalPrice.toLocaleString()}원</div>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="space-y-3">
                      {orderService.getOrderProgress(selectedOrder.id).map((progress, index) => (
                        <div key={index} className="flex items-start gap-3 bg-white border rounded-lg p-3">
                          <div className="text-xl">{getStatusIcon(progress.status)}</div>
                          <div className="flex-1">
                            <div className="font-medium text-sm">{progress.message}</div>
                            <div className="text-xs text-gray-500">{new Date(progress.timestamp).toLocaleString()}</div>
                            {progress.remainingTime && (
                              <div className="text-xs text-blue-600 font-medium">{progress.remainingTime}</div>
                            )}
                          </div>
