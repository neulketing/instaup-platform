...
export default function RechargeModal({
  ...
  const amountOptions = [
    ...
    { amount: 50000, bonus: 2500, label: '50,000원', bonusText: '+2,500원 보너스' },
    { amount: 100000, bonus: 7000, label: '100,000원', bonusText: '+7,000원 보너스' },
    { amount: 200000, bonus: 20000, label: '200,000원', bonusText: '+20,000원 보너스' },
    { amount: 500000, bonus: 60000, label: '500,000원', bonusText: '+60,000원 보너스' }
  ]
  ...
  const getChargeAmount = () => {
    const baseAmount = customAmount ? Number.parseInt(customAmount) || 0 : selectedAmount
    const bonusAmount = amountOptions.find(opt => opt.amount === selectedAmount)?.bonus || 0
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-lg w-full max-w-2xl max-h-screen overflow-y-auto">
        ...
        <div className="p-6">
          ...
          {/* 충전 금액 선택 */}
          <div className="mb-6">
            <h3 className="font-semibold text-gray-900 mb-4">💸 충전 금액 선택</h3>
                ...
                >
                  ...
                  {option.bonus > 0 && (
                    <div className="text-xs text-gray-500 mt-1">
                      실제 충전: {(option.amount + option.bonus).toLocaleString()}원
                    </div>
                  )}
                </button>
              ))}
            ...
            <div className="space-y-2 text-sm">
              <div className="flex justify-between">
                ...
                <span className="font-medium">
                  {(customAmount ? Number.parseInt(customAmount) || 0 : selectedAmount).toLocaleString()}원
                </span>
              </div>

              {!customAmount && selectedAmount > 0 && amountOptions.find(opt => opt.amount === selectedAmount)?.bonus && (
                <div className="flex justify-between">
                  <span className="text-green-600">보너스</span>
                  <span className="font-medium text-green-600">
                    +{amountOptions.find(opt => opt.amount === selectedAmount)?.bonus?.toLocaleString()}원
                  </span>
                </div>
              )}
              ...
              <div className="border-t pt-2">
                <div className="flex justify-between">
                  <span className="font-medium text-gray-900">실제 충전 금액</span>
              ...
              <div className="flex justify-between text-xs text-gray-500">
                <span>충전 후 잔액</span>
                <span>{(currentBalance + getNetAmount()).toLocaleString()}원</span>
              </div>
            </div>
          </div>
          ...
          <div className="mb-6">
            <label className="flex items-start gap-3 cursor-pointer">
              ...
              <div className="text-sm text-gray-600">
                <span className="font-medium text-gray-900">결제 약관에 동의합니다.</span>
                <div className="mt-1 text-xs">
                  • 충전된 금액은 환불이 불가능합니다<br/>
                  • 충전 금액은 서비스 이용에만 사용 가능합니다<br/>
                  • 부정 사용 적발 시 계정이 제재될 수 있습니다<br/>
          ...
          <button
            ...
            disabled={isProcessing || getNetAmount() < 1000 || !agreementChecked}
