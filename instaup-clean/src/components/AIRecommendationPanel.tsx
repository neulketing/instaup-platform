...
import { aiRecommendationService, type MarketingRecommendation, type UserProfile } from '../services/aiRecommendation'
...
export default function AIRecommendationPanel({ userSession, onServiceSelect }: AIRecommendationPanelProps) {
  ...
  const generateRecommendations = async () => {
    ...
    try {
      ...
      const recs = await aiRecommendationService.generatePersonalizedRecommendations(userProfile)
  ...
  const handleRecommendationClick = (recommendation: MarketingRecommendation) => {
    // 추천 클릭 이벤트 추적
    if (window.gtag) {
  ...
  const getTypeLabel = (type: string) => {
    switch (type) {
      case 'upsell': return '업그레이드 추천'
      case 'service': return '새 서비스 추천'
      case 'promo': return '프로모션'
      case 'retention': return '고객 유지'
      case 'reactivation': return '재활성화'
      default: return '추천'
    }
  }
    ...
    return (
      <div className="bg-gradient-to-br from-indigo-50 to-purple-50 rounded-lg p-6 border border-indigo-200">
        <div className="text-center">
          ...
          <h3 className="text-lg font-semibold text-gray-900 mb-2">AI 맞춤 추천</h3>
        ...
        <div className="p-4">
          {loading ? (
                ...
                >
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-2 mt-1">
                          <span className={`px-2 py-1 text-xs font-medium rounded-full border ${getUrgencyColor(rec.urgency)}`}>
                            {rec.urgency === 'high' ? '긴급' : rec.urgency === 'medium' ? '보통' : '낮음'}
                          </span>
                  ...
                  {(rec.discountPercent || rec.bonusAmount) && (
                    <div className="bg-green-50 border border-green-200 rounded-lg p-3 mb-3">
                      <div className="flex items-center gap-2">
                        <span className="text-green-600 font-medium">🎁 특별 혜택</span>
                        {rec.discountPercent && (
                  ...
                  {/* 추천 이유 */}
                  <div className="mb-3">
                    <div className="text-xs font-medium text-gray-700 mb-1">추천 이유:</div>
            ...
            <div className="text-center py-8">
              ...
              <p className="text-gray-600 mb-2">현재 추천할 서비스가 없습니다</p>
          ...
          <div className="mt-6 pt-4 border-t border-gray-100">
            <div className="flex items-center gap-2 text-xs text-gray-500">
              <span>🧠</span>
              <span>AI가 사용자의 주문 패턴, 선호도, 시장 트렌드를 분석하여 생성된 추천입니다.</span>
            </div>
          </div>
        </div>
      )}
