...
export default function CustomerSupportChat({
  ...
  const generateAutoResponse = (message: string, order?: OrderItem | null): ChatMessage => {
    const lowerMsg = message.toLowerCase()

    // 주문 관련 키워드 감지
    if (lowerMsg.includes('주문') || lowerMsg.includes('진행') || lowerMsg.includes('상태')) {
      if (order) {
        return {
        ...
        return {
          id: Date.now().toString() + '_response',
          type: 'agent',
          message: '주문 관련 문의는 구체적인 주문번호나 📎 주문첨부를 통해 알려주시면 더 정확한 안내가 가능해요! 어떤 주문에 대해 문의하시나요?',
          timestamp: new Date().toISOString()
        }
      }
    }
    ...
    // 리필 관련
    if (lowerMsg.includes('리필') || lowerMsg.includes('감소') || lowerMsg.includes('줄어')) {
      return {
        id: Date.now().toString() + '_response',
        type: 'agent',
    ...
    // 결제/환불 관련
    if (lowerMsg.includes('결제') || lowerMsg.includes('환불') || lowerMsg.includes('취소')) {
      return {
        id: Date.now().toString() + '_response',
        type: 'agent',
        message: '💳 결제/환불 관련 문의는 중요한 사안이에요.\n\n• 서비스 시작 전: 취소 및 환불 가능\n• 서비스 진행 중: 부분 환불 가능\n• 서비스 완료 후: 환불 불가\n\n구체적인 상황을 알려주시면 정확한 안내를 도와드릴게요!',
        timestamp: new Date().toISOString()
      }
    }

    // 계정/비밀번호 관련
    if (lowerMsg.includes('계정') || lowerMsg.includes('비밀번호') || lowerMsg.includes('로그인')) {
      return {
        id: Date.now().toString() + '_response',
        type: 'agent',
        message: '🔐 계정 관련 문의는 보안을 위해 추가 인증이 필요해요.\n\n• 비밀번호 재설정: 로그인 화면의 "비밀번호 찾기"\n• 계정 정보 변경: 계정설정 메뉴 이용\n• 기타 문제: 가입 시 이메일 주소로 본인 확인 후 도움\n\n어떤 문제가 있으신지 자세히 알려주세요!',
        timestamp: new Date().toISOString()
      }
    }
          ...
          {messages.map((msg) => (
            <div key={msg.id} className={`flex ${msg.type === 'user' ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-[80%] ${
                msg.type === 'user' ? 'bg-[#22426f] text-white' :
                msg.type === 'agent' ? 'bg-blue-50 text-blue-900' :
                ...
                {msg.attachments && msg.attachments.map((attachment, index) => (
                  <div key={index} className="mt-2 p-2 bg-white bg-opacity-20 rounded border">
            ...
            <div className="flex justify-start">
              <div className="bg-gray-100 rounded-lg p-3">
                <div className="flex items-center gap-1">
                  ...
                  <span className="text-xs text-gray-500 ml-2">상담사가 입력 중...</span>
                </div>
              </div>
            </div>
          )}
