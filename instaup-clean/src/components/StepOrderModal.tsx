...
export default function StepOrderModal({
  ...
  const steps: OrderStep[] = [
    { number: '01', title: '상품 설명 확인', description: '해당 상품에 대한 설명입니다. *꼭 읽어주세요*' },
    { number: '02', title: '주소 입력', description: '인스타그램 게시물 주소를 입력해주세요.' },
    { number: '03', title: '수량 입력', description: '구매하실 수량을 입력해 주세요.' },
    { number: '04', title: '주문 확인', description: '주문가격 (구매수량 입력시 자동으로 계산됩니다.)' }
  ]

  // SNS샵과 동일한 서비스 타입 (6x3 그리드)
  const serviceTypes = [
    { id: 'recommendation', name: '추천서비스', icon: '⭐', description: '인기상품', color: 'bg-yellow-50 border-yellow-200' },
    { id: 'event', name: '이벤트', icon: '🎁', description: '특가할인', color: 'bg-orange-50 border-orange-200' },
    { id: 'premium', name: '상위노출', icon: '👑', description: '프리미엄', color: 'bg-purple-50 border-purple-200' },
    { id: 'management', name: '계정관리', icon: '📊', description: '운영대행', color: 'bg-blue-50 border-blue-200' },
    { id: 'package', name: '패키지', icon: '📦', description: '맞춤상품', color: 'bg-green-50 border-green-200' },
    { id: 'instagram', name: '인스타그램', icon: '📷', description: '팔로워,좋아요', color: 'bg-pink-50 border-pink-200', highlight: true }
  ]

  // 인스타그램 세부 서비스
  const instagramServices = [
    { id: 'followers_basic', name: '인스타 팔로워 늘리기 [기본]', description: '안정적인 기본 팔로워', price: 120 },
    { id: 'followers_premium', name: '🟢 인스타 팔로워 늘리기 [프리미엄]', description: '고품질 프리미엄 팔로워', price: 180, highlight: true },
    { id: 'followers_vip', name: '👑 인스타 팔로워 늘리기 [VIP]', description: '최고급 VIP 팔로워', price: 250 },
    { id: 'likes', name: '인스타 좋아요 늘리기', description: '게시물 좋아요 증가', price: 80 },
    { id: 'views', name: '인스타 조회수 늘리기', description: '릴스/영상 조회수', price: 60 },
    { id: 'comments', name: '인스타 댓글 늘리기', description: '실제 댓글 작성', price: 200 },
    { id: 'story_views', name: '인스타 스토리 조회수', description: '스토리 조회수 증가', price: 90 }
  ]
  ...
  const productDetails: { [key: string]: ServiceItem } = {
    ...
    'followers_premium': {
      ...
      description: '고품질의 실제 한국인 팔로워로 구성된 프리미엄 서비스입니다.',
      price: 180,
      originalPrice: 220,
      discount: 18,
      minOrder: 50,
    ...
    'followers_vip': {
      ...
      description: '최고급 VIP 팔로워 서비스로 셀럽 수준의 팔로워를 제공합니다.',
      price: 250,
      originalPrice: 300,
      discount: 17,
      minOrder: 20,
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm opacity-90">실제 한국인 SNS 마케팅 서비스</p>
              </div>
            </div>
            <button
              onClick={onClose}
                ...
                <div key={index} className="flex items-center">
                  <div className="flex flex-col items-center">
                    <div className={`w-12 h-12 rounded-full flex items-center justify-center text-sm font-bold transition-all ${
                      currentStep > index + 1 ? 'bg-green-500 text-white shadow-lg' :
                      currentStep === index + 1 ? 'bg-[#22426f] text-white shadow-lg ring-4 ring-blue-200' :
                      'bg-gray-200 text-gray-500'
                    }`}>
                      {currentStep > index + 1 ? '✓' : step.number}
                    </div>
                    <div className={`text-xs mt-2 font-medium text-center ${
                      currentStep >= index + 1 ? 'text-[#22426f]' : 'text-gray-400'
                    }`}>
                      {step.title}
                    </div>
                  </div>
                  {index < steps.length - 1 && (
                    <div className={`w-20 h-1 mx-3 rounded ${
        ...
        <div className="flex">
          ...
          <div className="flex-1 p-8">
            ...
            {currentStep === 1 && selectedProduct && (
              <div className="space-y-6">
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-6">
                  <h3 className="text-lg font-bold text-blue-900 mb-2">📋 상품 설명 확인</h3>
                  <p className="text-blue-700">선택하신 상품의 상세 정보를 확인해주세요.</p>
                </div>

                <div className="bg-white rounded-xl border shadow-lg p-8">
                  <div className="space-y-6">
                    <div className="text-center">
                      <h4 className="text-xl font-bold text-gray-900 mb-4">{selectedProduct.name}</h4>
                      <p className="text-gray-600 mb-6">{selectedProduct.description}</p>

                      <div className="grid grid-cols-2 gap-4 text-sm">
                        <div className="bg-gray-50 p-4 rounded-lg">
                          <div className="font-medium text-gray-700">가격</div>
                          <div className="text-lg font-bold text-[#22426f]">{selectedProduct.price.toLocaleString()}원/100개</div>
                        </div>
                        <div className="bg-gray-50 p-4 rounded-lg">
                          <div className="font-medium text-gray-700">처리시간</div>
                          <div className="text-lg font-bold text-green-600">{selectedProduct.deliveryTime}</div>
                        </div>
                      </div>
            ...
            {currentStep === 2 && selectedProduct && (
              <div className="space-y-6">
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-6">
                  <h3 className="text-lg font-bold text-blue-900 mb-2">🔗 인스타그램 게시물 주소 입력</h3>
                  <p className="text-blue-700">서비스를 적용할 인스타그램 게시물의 정확한 URL을 입력해주세요.</p>
                </div>
                ...
                <div className="bg-white rounded-xl border shadow-lg p-8">
                  <div className="space-y-6">
                    ...
                    <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-6">
                      <h4 className="font-bold text-yellow-800 mb-3">📋 URL 입력 가이드</h4>
                    ...
                    <div className="bg-red-50 border border-red-200 rounded-lg p-6">
                      <h4 className="font-bold text-red-800 mb-3">⚠️ URL 입력 시 주의사항</h4>
                      <div className="space-y-2 text-sm text-red-700">
                        <div className="flex items-center gap-2">
                          <span className="text-red-500">•</span>
                          <span>반드시 <strong>정확한 게시물 링크</strong>를 입력해주세요</span>
                        </div>
                        <div className="flex items-center gap-2">
                          <span className="text-red-500">•</span>
                          <span>계정이 <strong>공개 상태</strong>인지 확인해주세요</span>
                        </div>
                        <div className="flex items-center gap-2">
                          <span className="text-red-500">•</span>
                          <span>잘못된 URL 입력 시 서비스가 진행되지 않습니다</span>
                        </div>
                        <div className="flex items-center gap-2">
                          <span className="text-red-500">•</span>
                          <span>프로필 링크가 아닌 <strong>게시물 링크</strong>여야 합니다</span>
                        </div>
                      </div>
                    </div>
                        ...
                        <div className="flex items-center gap-2">
                          {targetUrl.includes('instagram.com/p/') ? (
                            <>
                              ...
                              <span className="text-green-800 font-medium">올바른 인스타그램 게시물 URL입니다!</span>
                            </>
                          ) : (
                            <>
                              <span className="text-red-600 text-xl">❌</span>
                              <span className="text-red-800 font-medium">올바른 인스타그램 게시물 URL이 아닙니다.</span>
                            </>
                          )}
                        </div>
                      </div>
            ...
            {currentStep === 3 && selectedProduct && (
              <div className="space-y-6">
                <div className="bg-green-50 border border-green-200 rounded-lg p-6">
                  <h3 className="text-lg font-bold text-green-900 mb-2">📊 구매 수량 설정</h3>
                  <p className="text-green-700">
                    슬라이더를 이용해 수량을 선택하세요. 많이 구매할수록 더 큰 할인을 받을 수 있습니다!
                  </p>
                </div>
                ...
                <div className="bg-white rounded-xl border shadow-lg p-8">
                  <div className="space-y-8">
                    ...
                    <div className="bg-gradient-to-r from-red-50 to-pink-50 border border-red-200 rounded-lg p-6">
                      <h4 className="font-bold text-red-900 mb-4 text-lg">🎉 수량별 할인 혜택</h4>
                      <div className="grid grid-cols-2 gap-4 text-sm">
                        <div className="space-y-2">
                          <div className={`flex justify-between p-2 rounded ${quantity >= 5000 ? 'bg-red-100 border border-red-300' : 'bg-gray-50'}`}>
                            <span>5,000개 이상</span>
                            <span className="font-bold text-red-600">15% 할인</span>
                          </div>
                          <div className={`flex justify-between p-2 rounded ${quantity >= 2000 && quantity < 5000 ? 'bg-orange-100 border border-orange-300' : 'bg-gray-50'}`}>
                            <span>2,000개 이상</span>
                            <span className="font-bold text-orange-600">10% 할인</span>
                          </div>
                        </div>
                        <div className="space-y-2">
                          <div className={`flex justify-between p-2 rounded ${quantity >= 1000 && quantity < 2000 ? 'bg-yellow-100 border border-yellow-300' : 'bg-gray-50'}`}>
                            <span>1,000개 이상</span>
                            <span className="font-bold text-yellow-600">7% 할인</span>
                          </div>
                          <div className={`flex justify-between p-2 rounded ${quantity >= 500 && quantity < 1000 ? 'bg-green-100 border border-green-300' : 'bg-gray-50'}`}>
                            <span>500개 이상</span>
                            <span className="font-bold text-green-600">5% 할인</span>
                          </div>
                        </div>
                      </div>
                    </div>
                    ...
                    <div>
                      <label className="block text-lg font-bold text-gray-900 mb-6">
                        🎯 주문 수량 선택 <span className="text-red-500">*</span>
                      </label>
                      ...
                      <div className="mb-8">
                        ...
                        <div className="flex justify-between text-sm text-gray-500 mt-2">
                          <span>{selectedProduct.minOrder.toLocaleString()}개</span>
                          <span>{selectedProduct.maxOrder.toLocaleString()}개</span>
                        </div>
                      </div>

                      {/* 빠른 수량 선택 버튼 */}
                      <div className="grid grid-cols-5 gap-3 mb-6">
                        {[100, 500, 1000, 2000, 5000].filter(q =>
                          q >= selectedProduct.minOrder && q <= selectedProduct.maxOrder
                        ).map((quickQuantity) => (
                          <button
                            key={quickQuantity}
                    ...
                    <div className="bg-gradient-to-br from-blue-50 to-indigo-50 border border-blue-200 rounded-xl p-6">
                      <h4 className="font-bold text-blue-900 mb-4 text-lg">💰 주문 금액 계산</h4>
                      <div className="space-y-3">
                        <div className="flex justify-between text-lg">
                          <span className="text-gray-700">기본 단가 (100개당)</span>
                          <span className={getQuantityDiscount(quantity) > 0 ? 'line-through text-gray-400' : 'font-bold'}>
                            {selectedProduct.price.toLocaleString()}원
                          </span>
                        </div>
                        {getQuantityDiscount(quantity) > 0 && (
                          <div className="flex justify-between text-lg">
                            <span className="text-gray-700">할인 단가 (100개당)</span>
                            <span className="font-bold text-red-600">
                              {getDiscountedUnitPrice().toLocaleString()}원 (-{getQuantityDiscount(quantity)}%)
                            </span>
                          </div>
                        )}
                        <div className="flex justify-between text-lg">
                          <span className="text-gray-700">주문 수량</span>
                          <span className="font-bold">{quantity.toLocaleString()}개</span>
                        </div>
                        <div className="border-t border-blue-200 pt-3">
                          <div className="flex justify-between text-2xl">
                            <span className="font-bold text-blue-900">총 결제 금액</span>
                          ...
                          {getQuantityDiscount(quantity) > 0 && (
                            <div className="text-right text-sm text-red-600 mt-1">
                              기본가격 대비 {((selectedProduct.price * (quantity / 100)) - calculateTotalPrice()).toLocaleString()}원 절약!
                            </div>
                          )}
                        </div>
                      </div>
            ...
            {currentStep === 4 && selectedProduct && (
              <div className="space-y-6">
                <div className="bg-gradient-to-r from-green-50 to-emerald-50 border border-green-200 rounded-lg p-6">
                  <h3 className="text-xl font-bold text-green-900 mb-2">🛒 최종 주문 확인 및 결제</h3>
                  <p className="text-green-700">주문 정보를 최종 확인하고 결제를 진행해주세요.</p>
                </div>
                ...
                <div className="bg-white rounded-xl border shadow-lg p-8">
                  <div className="space-y-8">
                    ...
                    <div className="bg-blue-50 rounded-xl p-6 border border-blue-200">
                      <h4 className="font-bold text-blue-900 mb-4 text-lg">📦 주문 상품 정보</h4>
                      <div className="grid grid-cols-2 gap-6">
                        <div className="space-y-3">
                          ...
                          <div className="flex justify-between">
                            <span className="text-gray-600">단가 (100개당)</span>
                            <span className="font-bold">{selectedProduct.price.toLocaleString()}원</span>
                          </div>
                        </div>
                        <div className="space-y-3">
                          <div className="flex justify-between">
                            <span className="text-gray-600">처리 시간</span>
                            <span className="font-bold text-green-600">{selectedProduct.deliveryTime}</span>
                          </div>
                          <div className="flex justify-between">
                            <span className="text-gray-600">품질 등급</span>
                            <span className="font-bold uppercase">{selectedProduct.quality}</span>
                          </div>
                          <div className="flex justify-between">
                            <span className="text-gray-600">리필 보장</span>
                            <span className="font-bold text-purple-600">30일 무료</span>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="mt-4 pt-4 border-t border-blue-200">
                        <div className="flex justify-between">
                          ...
                          <span className="font-medium text-blue-600 break-all text-right max-w-md">{targetUrl}</span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="bg-green-50 rounded-xl p-6 border border-green-200">
                      <h4 className="font-bold text-green-900 mb-4 text-lg">💳 결제 정보</h4>
                      <div className="space-y-4">
                        <div className="flex justify-between text-xl">
                          <span className="font-bold">총 주문 금액</span>
                          <span className="font-bold text-[#22426f] text-2xl">{calculateTotalPrice().toLocaleString()}원</span>
                        </div>
                        ...
                        <div className="border-t border-green-200 pt-4">
                          <div className="grid grid-cols-2 gap-4 text-sm">
                            <div className="flex justify-between">
                              ...
                              <span className={`font-bold ${userBalance >= calculateTotalPrice() ? 'text-green-600' : 'text-red-600'}`}>
                            ...
                            <div className="flex justify-between">
                              ...
                              <span className="font-bold">
                                {(userBalance - calculateTotalPrice()).toLocaleString()}원
                              </span>
                            </div>
                          </div>
                        </div>
                        ...
                        {userBalance < calculateTotalPrice() && (
                          <div className="mt-4 p-4 bg-red-50 border border-red-200 rounded-lg">
                            <div className="flex items-center gap-2">
                              ...
                              <div>
                                <p className="text-red-800 font-bold">잔액이 부족합니다!</p>
                                <p className="text-red-600 text-sm">
                                  {(calculateTotalPrice() - userBalance).toLocaleString()}원을 추가로 충전해주세요.
                                </p>
                              </div>
                            </div>
                          </div>
                    ...
                    <div className="bg-yellow-50 rounded-xl p-6 border border-yellow-200">
                      <h4 className="font-bold text-yellow-800 mb-4 text-lg">📋 최종 확인사항</h4>
                      <div className="grid grid-cols-2 gap-4 text-sm">
                        <div className="space-y-2">
                          <div className="flex items-center gap-2">
                            <span className="text-yellow-600">✓</span>
                            <span className="text-yellow-700">주문 후 <strong>즉시 서비스 시작</strong> (1-5분 내)</span>
                          </div>
                          <div className="flex items-center gap-2">
                            <span className="text-yellow-600">✓</span>
                            <span className="text-yellow-700">서비스 시작 후 <strong>취소/환불 불가</strong></span>
                          </div>
                        </div>
                        <div className="space-y-2">
                          <div className="flex items-center gap-2">
                            <span className="text-yellow-600">✓</span>
                            <span className="text-yellow-700"><strong>30일간 무료 리필</strong> 보장</span>
                          </div>
                          <div className="flex items-center gap-2">
                            <span className="text-yellow-600">✓</span>
                            <span className="text-yellow-700">계정은 <strong>공개 상태 유지</strong> 필수</span>
                          </div>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="text-center">
                      <button
                        ...
                        disabled={isLoading || userBalance < calculateTotalPrice()}
            ...
            <div className="flex justify-between mt-12 pt-8 border-t border-gray-200">
              ...
              {currentStep < 4 ? (
                <button
                  onClick={handleNext}
                  disabled={
                    (currentStep === 2 && (!targetUrl || !targetUrl.includes('instagram.com/p/')))
                  }
          ...
          <div className="w-96 bg-gray-50 p-6 border-l">
            ...
            <div className="bg-[#22426f] text-white p-6 rounded-lg mb-6 text-center">
              ...
              <p className="text-sm opacity-90">마지막 업데이트: {new Date().toLocaleString()}</p>
            ...
            <div className="bg-white rounded-lg p-6 mb-6 border">
              ...
              <div className="space-y-4 text-sm">
                <div className="p-3 bg-blue-50 rounded-lg border-l-4 border-blue-500">
                  ...
                  <p className="text-blue-800">인스타그램 팔로워 서비스 품질이 업그레이드되었습니다.</p>
                </div>
                ...
                <div className="p-3 bg-green-50 rounded-lg border-l-4 border-green-500">
                  ...
                  <p className="text-green-800">서버 최적화 작업이 완료되어 더욱 빨라졌습니다.</p>
                </div>
                ...
                <div className="p-3 bg-purple-50 rounded-lg border-l-4 border-purple-500">
                  ...
                  <p className="text-purple-800">첫 주문 시 20% 할인 이벤트가 진행 중입니다.</p>
                </div>
              </div>
            </div>
