...
export default function OrderModal({
  ...
  const originalTotalPrice = service.originalPrice ? Math.floor((orderData.quantity / 1) * service.originalPrice) : null
  ...
  const validateInstagramLink = (input: string) => {
    ...
    // 인스타그램 링크 패턴
    const linkPattern = /(?:https?:\/\/)?(?:www\.)?instagram\.com\/([a-zA-Z0-9_.]{1,30})/
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (orderData.quantity < service.minOrder || orderData.quantity > service.maxOrder) {
      alert(`주문 수량은 ${service.minOrder}개 이상 ${service.maxOrder}개 이하로 입력해주세요.`)
      return
    }
  ...
  const handleQuantityChange = (value: number) => {
    const clampedValue = Math.max(service.minOrder, Math.min(service.maxOrder, value))
              ...
              <div className="ml-8">
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    onChange={(e) => setOrderData(prev => ({ ...prev, link: e.target.value, isValidated: false }))}
                  ...
                  >
                    {isLoading ? '검증중...' : orderData.isValidated ? '완료' : '검증'}
                  </button>
                </div>
                ...
                {linkValidation.message && (
                  <div className={`mt-2 p-3 rounded-lg text-sm ${
                    ...
                    {linkValidation.isValid ? '✅' : '❌'} {linkValidation.message}
                  </div>
                )}
            ...
            <div className={`${validationStep >= 2 ? 'block' : 'hidden'}`}>
              ...
              <div className="ml-8">
                <div className="flex items-center gap-3 mb-2">
                  <button
                    type="button"
                    onClick={() => handleQuantityChange(orderData.quantity - 10)}
                ...
                <p className="text-xs text-gray-500">
                  최소 {service.minOrder.toLocaleString()}개 ~ 최대 {service.maxOrder.toLocaleString()}개
                </p>
              </div>
            </div>
            ...
            <div className="p-6 bg-gray-50 border-t">
              ...
              {isLoggedIn && (
                <div className="mb-4 p-3 bg-white rounded-lg border">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="font-medium text-[#22426f]">{userBalance.toLocaleString()}원</span>
                  </div>
                  {hasInsufficientBalance && (
                    <div className="mt-2 text-xs text-red-600">
                      ⚠️ 잔액이 부족합니다. ({(totalPrice - userBalance).toLocaleString()}원 부족)
                    </div>
                  )}
                </div>
              )}
              ...
              <div className="space-y-2 mb-4">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-medium">{orderData.quantity.toLocaleString()}개</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-gray-600">단가</span>
                  <span className="font-medium">{service.price.toLocaleString()}원/개</span>
                </div>
                {savings > 0 && (
                  <>
              ...
              <div className="text-center text-sm text-gray-600 mb-4">
                <span className="font-medium">⏰ 시작 시간:</span> {service.deliveryTime}
                <br />
                <span className="font-medium">🔄 리필 정책:</span> 60일간 3회 무료 리필
              </div>

              {/* 주문 버튼 */}
              <button
                type="submit"
                disabled={isLoading || !orderData.isValidated || hasInsufficientBalance}
