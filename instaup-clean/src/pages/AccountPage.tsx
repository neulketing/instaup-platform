...
export default function AccountPage() {
  ...
  const [activeTab, setActiveTab] = useState<'profile' | 'password' | 'referral'>('profile')
    ...
    try {
      ...
      if (response.success && response.data) {
        const profile: ProfileData = {
          ...
          referralCode: userSession!.referralCode || 'INSTA' + Math.random().toString(36).substr(2, 6).toUpperCase(),
  ...
  const handleProfileUpdate = async () => {
    ...
    try {
      ...
      if (response.success) {
        ...
        setMessage({ type: 'error', text: response.error || '프로필 업데이트에 실패했습니다.' })
      }
    } catch (error: any) {
  ...
  const handlePasswordChange = async () => {
    ...
    try {
      ...
      if (response.success) {
        ...
        setMessage({ type: 'error', text: response.error || '비밀번호 변경에 실패했습니다.' })
      }
    } catch (error: any) {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600 mb-4">계정 관리를 하려면 로그인해주세요.</p>
          <button
            onClick={() => navigate('/')}
          ...
          <div className="lg:col-span-3">
            {loading ? (
              <div className="bg-white rounded-lg shadow-sm p-8 text-center">
                ...
                <p className="mt-2 text-gray-600">계정 정보를 불러오고 있습니다...</p>
              </div>
            ) : (
              <div className="bg-white rounded-lg shadow-sm p-6">
                ...
                {activeTab === 'profile' && (
                  <div>
                    <h2 className="text-lg font-semibold text-gray-900 mb-6">👤 프로필 관리</h2>
                    ...
                    <div className="space-y-6">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                          ...
                          <input
                            type="text"
                            value={profileData ? new Date(profileData.joinDate).toLocaleDateString('ko-KR') : ''}
                            disabled
                            className="w-full px-3 py-2 border border-gray-300 rounded-lg bg-gray-50 text-gray-500"
                          />
                        </div>
                      ...
                      {editingProfile && (
                        <div className="flex gap-2">
                          ...
                          <button
                            onClick={() => {
                              setEditingProfile(false)
                              setProfileForm({ nickname: profileData?.nickname || '' })
                            }}
                ...
                {activeTab === 'referral' && (
                  <div>
                    <h2 className="text-lg font-semibold text-gray-900 mb-6">🎁 추천인 관리</h2>

                    <div className="space-y-6">
                      {/* 나의 추천인 코드 */}
                      <div>
                        <h3 className="text-md font-medium text-gray-900 mb-4">나의 추천인 코드</h3>
                      ...
                      {/* 추천인 혜택 */}
                      <div>
                        <h3 className="text-md font-medium text-gray-900 mb-4">추천인 혜택</h3>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                            <div className="text-green-800">
                              <div className="font-medium mb-1">💰 추천인 보상</div>
                              <div className="text-sm">
                                • 신규 회원 가입 시: 10,000원<br/>
                                • 추천 받은 회원 첫 주문 시: 주문금액의 5%
                              </div>
                            </div>
                          </div>
                          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                            <div className="text-blue-800">
                              <div className="font-medium mb-1">🎯 누적 혜택</div>
                              <div className="text-sm">
                                • 추천 인원 10명 달성: 추가 50,000원<br/>
                                • 추천 인원 50명 달성: 추가 300,000원
                              </div>
                            </div>
                          </div>
                        </div>
                      ...
                      {/* 추천 통계 (임시 데이터) */}
                      <div>
                        <h3 className="text-md font-medium text-gray-900 mb-4">추천 통계</h3>
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                          <div className="text-center p-4 border border-gray-200 rounded-lg">
                            <div className="text-2xl font-bold text-gray-900">0</div>
                            <div className="text-sm text-gray-600">총 추천인원</div>
                          </div>
                          <div className="text-center p-4 border border-gray-200 rounded-lg">
                            <div className="text-2xl font-bold text-gray-900">0원</div>
                            <div className="text-sm text-gray-600">추천 수익</div>
                          </div>
                          <div className="text-center p-4 border border-gray-200 rounded-lg">
                            <div className="text-2xl font-bold text-gray-900">0</div>
                          ...
                          <div className="text-center p-4 border border-gray-200 rounded-lg">
                            <div className="text-2xl font-bold text-gray-900">10,000원</div>
                            <div className="text-sm text-gray-600">가입 보너스</div>
                          </div>
                        </div>
                      </div>
                    </div>
