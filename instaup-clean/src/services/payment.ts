...
export interface PaymentRequest {
  ...
  paymentMethod: 'card' | 'kakaopay' | 'naverpay' | 'tosspay' | 'bank' | 'phone'
}
...
class PaymentService {
  ...
  // 토스페이먼츠 결제 처리
  async processTossPayment(paymentRequest: PaymentRequest): Promise<PaymentResponse> {
    try {
      // 토스페이먼츠 SDK 동적 로드
  ...
  // 카카오페이 결제 처리
  async processKakaoPayment(paymentRequest: PaymentRequest): Promise<PaymentResponse> {
    try {
      // 카카오페이 결제 준비 요청
  ...
  // 네이버페이 결제 처리
  async processNaverPayment(paymentRequest: PaymentRequest): Promise<PaymentResponse> {
    try {
      // 네이버페이 SDK 또는 API 호출
      const naverPayData = {
  ...
  // 무통장입금 처리
  async processBankTransfer(paymentRequest: PaymentRequest): Promise<PaymentResponse> {
    try {
      // 가상계좌 발급 또는 무통장입금 안내
  ...
  // 휴대폰결제 처리
  async processPhonePayment(paymentRequest: PaymentRequest): Promise<PaymentResponse> {
    try {
  ...
  private validatePayment(paymentRequest: PaymentRequest): {
    ...
    const provider = this.providers.find(p =>
      p.methods.includes(paymentRequest.paymentMethod) && p.isActive
    )
  ...
  // 결제 취소
  async cancelPayment(paymentKey: string, cancelReason: string): Promise<{
    success: boolean
    message: string
  }> {
    try {
  ...
  calculateFee(amount: number, method: string): number {
    const provider = this.providers.find(p =>
      p.methods.includes(method) && p.isActive
    )
