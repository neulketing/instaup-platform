...
const initializeSupabase = () => {
  if (!SUPABASE_URL || !SUPABASE_ANON_KEY) {
    console.warn('Supabase 환경변수가 설정되지 않았습니다. 일부 기능이 제한됩니다.')
    return null
  }
...
export interface NotificationData {
  id: string
  userId: string
  type: 'order_update' | 'payment_success' | 'payment_failed' | 'system_message' | 'promotion'
  title: string
  message: string
  data?: any
  read: boolean
...
class SupabaseService {
  ...
  async uploadFile(
    ...
    if (!this.client) {
      return { url: null, error: 'Supabase 클라이언트가 초기화되지 않았습니다.' }
    }
...
export const useRealtimeNotifications = (userId: string | null) => {
  ...
  useEffect(() => {
    ...
    // 실시간 구독
    subscription = supabaseService.subscribeToNotifications(userId, (newNotification) => {
  ...
  const markAsRead = async (notificationId: string) => {
    const success = await supabaseService.markNotificationAsRead(notificationId)
    if (success) {
