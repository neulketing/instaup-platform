// 백엔드 API 연동 서비스
import { type OrderResponse, OrderRequest } from './api'
...
class BackendApiService {
  ...
  // 주문 생성
  async createOrder(orderRequest: { serviceId: string; quantity: number; targetUrl: string }) {
  ...
  async getOrders(filters?: {
    ...
    return this.makeRequest<{ orders: Order[]; total: number; page: number }>(`/order${query}`)
  }
  ...
  // 토스페이 결제 확인
  async confirmTossPayment(paymentKey: string, orderId: string, amount: number): Promise<ApiResponse<any>> {
  ...
  // 모든 사용자 조회 (관리자)
  async getAllUsers(page = 1, limit = 20): Promise<ApiResponse<{ users: User[]; total: number }>> {
    return this.makeRequest<{ users: User[]; total: number }>(`/admin/users?page=${page}&limit=${limit}`)
  }
  ...
  async getAllOrders(filters?: {
    ...
    const query = params.toString() ? `?${params.toString()}` : ''
    return this.makeRequest<{ orders: Order[]; total: number }>(`/admin/orders${query}`)
  }
  ...
  private handleRealtimeUpdate(data: any) {
    switch (data.type) {
      ...
      case 'balance_update':
        // 잔액 업데이트
        window.dispatchEvent(new CustomEvent('balanceUpdate', { detail: data.payload }))
        break
      case 'notification':
        // 일반 알림
