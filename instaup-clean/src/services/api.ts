...
class SNSAPIService {
  ...
  // Instagram API 연동
  async processInstagramOrder(orderRequest: OrderRequest): Promise<OrderResponse> {
    try {
      ...
      return {
        ...
        estimatedCompletion: this.calculateEstimatedCompletion(orderRequest.quantity)
      }
    } catch (error) {
  ...
  // YouTube API 연동
  async processYouTubeOrder(orderRequest: OrderRequest): Promise<OrderResponse> {
    try {
  ...
  // 범용 SNS 플랫폼 API 호출
  private async callProvider(platform: string, orderRequest: OrderRequest): Promise<any> {
    const provider = this.getOptimalProvider(platform)

    if (!provider) {
      throw new Error(`${platform} 플랫폼을 지원하는 API 제공업체를 찾을 수 없습니다.`)
    }
  ...
  // 계정 유효성 검증
  async validateAccount(platform: string, accountUrl: string): Promise<{
    isValid: boolean
    isPublic: boolean
  ...
  async processRefill(orderId: string): Promise<{
    ...
    try {
      const response = await fetch(`${this.config.baseUrl}/orders/${orderId}/refill`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${this.config.apiKey}`
        }
  ...
  async checkAPIHealth(): Promise<{
    ...
    const healthyProviders = Object.values(providerStatus).filter(status => status).length
