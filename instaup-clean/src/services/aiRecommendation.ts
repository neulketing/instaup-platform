...
class AIRecommendationService {
  ...
  constructor() {
    ...
    this.baseUrl = import.meta.env.VITE_AI_SERVICE_URL || 'https://api.openai.com/v1'
  }

  // 사용자별 개인화 추천
  async generatePersonalizedRecommendations(userProfile: UserProfile): Promise<MarketingRecommendation[]> {
    try {
      // AI 모델을 통한 개인화 추천 생성
    ...
    return [
      {
        ...
        variants: [
          { name: '현재 (5%)', description: '50,000원 이상 5% 보너스', expectedImpact: 0 },
          { name: '변형 A (7%)', description: '50,000원 이상 7% 보너스', expectedImpact: 15 },
          { name: '변형 B (단계별)', description: '금액별 차등 보너스', expectedImpact: 25 }
        ],
      ...
      },
      {
        ...
        variants: [
          { name: '파란색 (현재)', description: '기존 파란색 버튼', expectedImpact: 0 },
          { name: '주황색', description: '주황색 버튼으로 변경', expectedImpact: 12 },
          { name: '그라데이션', description: '그라데이션 효과 적용', expectedImpact: 8 }
        ],
  ...
  // Mock 데이터 생성 함수들
  private generateMockRecommendations(userProfile: UserProfile): MarketingRecommendation[] {
    const baseRecommendations = [
      {
        id: 'rec_001',
        type: 'upsell' as const,
        title: 'Instagram 프리미엄 팔로워 업그레이드',
        description: '현재 사용 중인 일반 팔로워 서비스를 프리미엄으로 업그레이드하시면 더 높은 품질의 팔로워를 확보할 수 있습니다.',
      ...
      },
      {
        ...
        description: 'Instagram에서 성공을 경험하셨다면, YouTube에서도 채널을 키워보세요. 첫 주문 시 특별 할인을 제공합니다.',
      ...
      },
      {
        ...
        title: '대량 주문 VIP 혜택',
        description: '충성 고객님을 위한 특별 혜택! 10만원 이상 주문 시 추가 보너스와 우선 처리 서비스를 제공합니다.',
    ...
    return baseRecommendations.map(rec => ({
      ...rec,
      expectedRevenue: Math.floor(rec.expectedRevenue * (0.8 + Math.random() * 0.4))
    }))
  }
    ...
    return [
      {
        ...
        description: 'TikTok 팔로워 서비스 수요가 지난주 대비 25% 증가했습니다. 마케팅 집중으로 시장 점유율을 확대할 기회입니다.',
        impact: 'high',
        actionRequired: true,
        suggestedActions: [
          'TikTok 서비스 프로모션 실시',
      ...
      },
      {
        type: 'warning',
        title: '결제 실패율 증가',
        description: '휴대폰 결제의 실패율이 15%로 상승했습니다. 사용자 경험 개선이 필요합니다.',
        impact: 'medium',
        actionRequired: true,
        suggestedActions: [
          '결제 프로세스 간소화',
      ...
      },
      {
        type: 'optimization',
        title: '가격 최적화 기회',
        description: 'Instagram 프리미엄 서비스의 마진을 5% 조정하면 월 300만원 추가 수익이 가능합니다.',
        impact: 'medium',
...
export const useAIRecommendations = () => {
  return {
    generatePersonalized: aiRecommendationService.generatePersonalizedRecommendations.bind(aiRecommendationService),
    analyzeMarketTrends: aiRecommendationService.analyzeMarketTrends.bind(aiRecommendationService),
    generateInsights: aiRecommendationService.generateBusinessInsights.bind(aiRecommendationService),
    optimizePricing: aiRecommendationService.optimizePricing.bind(aiRecommendationService),
    generatePromotions: aiRecommendationService.generateAutoPromotions.bind(aiRecommendationService),
    suggestABTests: aiRecommendationService.suggestABTests.bind(aiRecommendationService)
  }
}
